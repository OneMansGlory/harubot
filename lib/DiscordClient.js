...
class DiscordClient {
  ...
  init () {
    ...
    client.on('message', msg => {
      ...
      if (command === 'no perms') {
        client.reply(msg, 'you don\'t have the permissions to do that, lol.');
      } else if (command) {
    ...
    client.login(this.email, this.pass).then(success).catch(err);
    function success (token) {
      if (typeof config.get('discord-token') === 'undefined') {
